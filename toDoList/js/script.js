...
function valid() {
    ...
    var textAdd = document.getElementById("textAdd"),
        ...
        parent = boardsArray[0].childNodes[3],
        sibling = parent.childNodes[3];
    
    if(error === false) {
        if(textAdd.value.charAt(0) === "") {
            
            errorMsg.classList.add("board__header--error");
            errorMsg.innerHTML = "Write something";
            parent.insertBefore(errorMsg, sibling);
    ...
    for(var i = 0; i < textAdd.value.length; i++) {
        if(textAdd.value.charAt(i) !== " ") {
            pass = true;
        }
...
function delayAndCreate(boardNumber, Validator, itemValue, direction) {
    
    
    setTimeout(function() {
        
        createItem(boardNumber, Validator, itemValue, direction); 
    }, 51);  
...
function createItem(boardNumber, Validator, itemValue, direction) {
    ...
    if(boardNumber === 0 && Validator === true) {
    ...
    else if(boardNumber === 0 && Validator === false) {
    ...
    else if(boardNumber === 1 && Validator === false) {
        if(direction === "right") {
    ...
    else if(boardNumber === 2 && Validator === false) {
